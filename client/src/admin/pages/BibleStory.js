...
function BibleStory() {
	...
	const getKey = (pageIndex, previousPageData) => {
		...
		if (pageIndex === 0)
			return { path: "/bible-stories", cursor: 0, limit: PAGE_SIZE };
	...
	const handleSubmit = async (e) => {
		e.preventDefault();
		try {
			if (title.trim() === "" || description.trim() === "" || photo === null) {
				handleClose();
				return Swal.fire({
					text: "Please enter all Bible Story Details",
					animation: true,
			...
			await Axios.post("/bible-stories", body);
			clearInput();
			handleClose();
			Swal.fire({
				text: "Bible Story data sent successfully",
				icon: "success",
				animation: true,
				confirmButtonColor: "#0000FF",
			}).then(() => {
	...
	const handleDelete = async (e, id) => {
		...
		try {
			...
			Swal.fire({
				...
				if (result.value) {
					try {
						const res = await Axios.delete(`/bible-stories/${id}`);
						setSize((pre) => pre + 1);
						Swal.fire({
	...
	const handleEditSubmit = async (e, currentData) => {
		...
		try {
			...
			const res = await Axios.patch(`/bible-stories/${currentData.id}`, body);
			if (res.status === 200) {
			...
			handleCloseEdit();
			Swal.fire({
				text: "Bible Story Edited Successfully",
				icon: "success",
				animation: true,
				confirmButtonColor: "#0000FF",
				timer: 3000,
						...
						<Grid container spacing={3}>
							<Grid item xs={12}>
								<Paper sx={{ p: 2, display: "flex", flexDirection: "column" }}>
									<Title>
										Bible Story:(<b>{totalBibleStory}</b>)
									</Title>
