...
import { getBoardOfTrusteesSWRCursor } from "../../lib/boardOfTrustees";
...
export default function BoardOfTrustees() {
	...
	const getKey = (pageIndex, previousPageData) => {
		...
		if (pageIndex === 0)
			return { path: "/board-of-trustees", cursor: 0, limit: PAGE_SIZE };
	...
	const { data, size, setSize, error } = useSWRInfinite(
		getKey,
		getBoardOfTrusteesSWRCursor
	);
	const isLoading = !data && !error;
	const isLoadingMore =
		isLoading || (size > 0 && data && typeof data[size - 1] === "undefined");
	const allBoardOfTrustees = data
		? data.reduce((prev, curr) => [...prev, ...curr.data], [])
		: [];
	const totalBoardOfTrustees = allBoardOfTrustees.length;
	...
	const handleSubmit = async (e) => {
		...
		try {
			if (
				...
				handleClose();
				return Swal.fire({
					text: "Please enter all Board of Trustees Details",
					animation: true,
			...
			await Axios.post("/board-of-trustees", body);
			clearInput();
			handleClose();
			Swal.fire({
				text: "Board of Trustee data sent successfully",
				icon: "success",
				animation: true,
				confirmButtonColor: "#0000FF",
			}).then(() => {
	...
	const handleDelete = async (e, id) => {
		...
		try {
			...
			Swal.fire({
				...
				if (result.value) {
					try {
						const res = await Axios.delete(`/board-of-trustees/${id}`);
						setSize((pre) => pre + 1);
						Swal.fire({
	...
	const handleEditSubmit = async (e, currentData) => {
		...
		try {
			...
			const res = await Axios.patch(
				`/board-of-trustees/${currentData.id}`,
				body
			);
			...
			handleCloseEdit();
			Swal.fire({
				text: "Board of Trustees Edited Successfully",
				icon: "success",
				animation: true,
				confirmButtonColor: "#0000FF",
				timer: 3000,
						...
						<Grid container spacing={3}>
							<Grid item xs={12}>
								<Paper sx={{ p: 2, display: "flex", flexDirection: "column" }}>
									<Title>All Board of Trustees</Title>
									<Stack spacing={2}>
										<Item></Item>
										<Item>
											<Button
												...
												startIcon={<Add />}
											>
												Add Board of Trustees
											</Button>
										</Item>
									</Stack>
									{/* Add Board of Trustees Modal */}
									<Modal
										open={open}
										onClose={handleClose}
									...
									>
										<Box sx={modalStyles}>
											...
											<Input
												...
												placeholder="Enter Board of Trustee Title"
												value={title}
									...
									>
										<Box sx={modalStyles}>
											...
											<Input
												...
												name="title"
												className="bordered form-control"
												placeholder="Enter Board of Trustees Title"
												value={titleEdit}
