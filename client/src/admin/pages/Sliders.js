...
const Sliders = () => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			if (
				...
				handleClose();
				return Swal.fire({
					text: "Please enter all Slider Details",
					animation: true,
			...
			await Axios.post("/slider", body);
			clearInput();
			handleClose();
			Swal.fire({
				text: "Slider data sent successfully",
				icon: "success",
				animation: true,
				confirmButtonColor: "#0000FF",
			}).then(() => {
	...
	const handleDelete = async (e, id) => {
		...
		try {
			...
			Swal.fire({
				...
				if (result.value) {
					try {
						const res = await Axios.delete(`/slider/${id}`);
						setSize((pre) => pre + 1);
						Swal.fire({
	...
	const handleEditSubmit = async (e, currentData) => {
		...
		try {
			...
			const res = await Axios.patch(`/slider/${currentData.id}`, body);
			console.log("res client", res);
			clearInput();
			handleCloseEdit();
			Swal.fire({
				text: "Slider Edited Successfully",
				icon: "success",
				animation: true,
				confirmButtonColor: "#0000FF",
				timer: 3000,
								...
								<Paper sx={{ p: 2, display: "flex", flexDirection: "column" }}>
									...
									>
										<Box sx={modalStyles}>
											...
											/>
											<label className="mt-4">Page:</label>
											{/* <Input
												type="text"
												label="Page"
												id="page"
											...
											<FormControl fullWidth>
												...
												>
													...
													<MenuItem value="Chapel Project">
														Chapel Project
													</MenuItem>
													<MenuItem value="Mission Project">
														Mission Project
													</MenuItem>
												</Select>
											</FormControl>
									...
									<Modal
										...
										aria-describedby="modal-modal-description"
									>
										<Box sx={modalStyles} spacing="20px">
											<label className="mt-4">Title:</label>
											<Input
												type="text"
												label="Title"
