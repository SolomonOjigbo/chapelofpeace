...
function LiveStreaming() {
	...
	console.log("allLiveStreaming>>>>>>", allLiveStreaming);
	...
	const handleSubmit = async (e) => {
		...
		try {
			if (
				...
				handleClose();
				return Swal.fire({
					text: "Please enter all Live Streaming Details",
					animation: true,
			...
			await Axios.post("/live-streaming", body);
			clearInput();
			handleClose();
			Swal.fire({
				text: "Live Streaming data sent successfully",
				icon: "success",
				animation: true,
				confirmButtonColor: "#0000FF",
			}).then(() => {
	...
	const handleDelete = async (e, id) => {
		...
		try {
			...
			Swal.fire({
				...
				if (result.value) {
					try {
						const res = await Axios.delete(`/live-streaming/${id}`);
						setSize((pre) => pre + 1);
						Swal.fire({
	...
	const handleEditSubmit = async (e, currentData) => {
		...
		try {
			...
			const res = await Axios.patch(`/live-streaming/${currentData.id}`, body);
			console.log("res client", res);
			clearInput();
			handleCloseEdit();
			Swal.fire({
				text: "Live Streaming Edited Successfully",
				icon: "success",
				animation: true,
				confirmButtonColor: "#0000FF",
				timer: 3000,
						...
						<Grid container spacing={3}>
							<Grid item xs={12}>
								<Paper sx={{ p: 2, display: "flex", flexDirection: "column" }}>
									<Title>
										Live Streaming:(<b>{totalLiveStreaming}</b>)
									</Title>
									...
									>
										<Box sx={modalStyles}>
											...
											<Input
												...
												placeholder="Enter Live Streaming Title"
												value={title}
											...
											<Input
												...
												name="description"
												className="bordered form-control"
												placeholder="Enter Live Streaming Description"
												value={description}
