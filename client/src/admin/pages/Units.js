...
import { getUnitSWRCursor } from '../../lib/unit'
...
const Units = () => {
  ...
  const getKey = (pageIndex, previousPageData) => {
    ...
    if (pageIndex === 0) return { path: '/unit', cursor: 0, limit: PAGE_SIZE }
  ...
  const allUnit = data
    ...
    : []
  const totalUnit = allUnit.length
...
const isReachingEnd =
  ...
  const handleSubmit = async e => {
    e.preventDefault()
    try {
      if (name.trim() === '' || photo === null) {
        handleClose()
        return Swal.fire({
          text: 'Please enter all Unit Details',
          animation: true,
      ...
      await Axios.post('/unit', body)
      clearInput()
      handleClose()
      Swal.fire({
        text: 'Unit data sent successfully',
        icon: 'success',
        animation: true,
        confirmButtonColor: '#0000FF'
      }).then(() => {
  ...
  const handleEditSubmit = async (e, currentData) => {
    ...
    try {
      ...
      if (nameEdit.trim() === '' || photoEdit === null) {
        handleCloseEdit()
        return Swal.fire({
          text: 'Please enter all Unit Details',
          animation: true,
      ...
      const res = await Axios.patch(`/unit/${currentData.id}`, body)
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column' }}>
                  ...
                  >
                    <Box sx={modalStyles}>
                      <label className='mt-4'>Unit Name:</label>
                      <Input
                        type='text'
                        label='Name'
                        id='name'
