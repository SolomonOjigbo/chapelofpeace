...
const getPartner = async (payload) => {
  try {
    ...
    const result = await PartnersModel.findAll({
    ...
    console.log('All Partners', result)
...
const deletePartner = async payload => {
  try {
    ...
    const record = await PartnersModel.findByPk(payload)
...
const updatePartner = async ({ id, update }) => {
  try {
    console.log('model params', id, update)
    const updatedRecord = await PartnersModel.update(update, {
      where: { id },
      returning: true
    })
    const updatedData = await PartnersModel.findOne({ where: { id } })
...
module.exports = { insertPartner, getPartner, deletePartner, updatePartner }
