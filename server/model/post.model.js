...
const deletePost = async (payload) => {
	try {
		...
		const record = await PostModel.findByPk(payload);
...
const updatePost = async ({ id, update }) => {
	try {
		...
		const updatedRecord = await PostModel.update(update, {
			where: { id },
			returning: true,
		});
		const updatedData = await PostModel.findOne({ where: { id } });
...
const getSinglePost = async (payload) => {
	try {
		// console.log("model params:", payload);
		const getSinglePost = await PostModel.findByPk(payload);
...
module.exports = { insertPost, getPost, getSinglePost, deletePost, updatePost };
