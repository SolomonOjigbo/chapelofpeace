...
const getMeditations = async (payload) => {
  try {
    ...
    const result = await MeditationModel.findAll({
    ...
    console.log('All Meditations', result)
...
const deleteMeditations = async payload => {
  try {
    console.log('model params', payload)
    // Find the record to delete
    const record = await MeditationModel.findByPk(payload)
...
const updateMeditations = async ({ id, update }) => {
  try {
    console.log('model params', id, update)
    const updatedRecord = await MeditationModel.update(update, {
      where: { id },
      returning: true
    })
    const updatedData = await MeditationModel.findOne({ where: { id } })
...
module.exports = { insertMeditations, getMeditations, deleteMeditations, updateMeditations }
